/**
 * Implements a program which encrypts and then decrypts text using Vigenère
 * cipher. The Vigenère cipher is a method of encrypting alphabetic text by
 * using a series of interwoven Caesar ciphers based on the letters of a
 * keyword. Refer : https://en.wikipedia.org/wiki/Vigen%C3%A8re_cipher
 *
...
std::string encrypt_data(std::string plain_text, std::string key) {
  ...
  for (unsigned int i = 0; i < text_len; ++i) {
    if (isalpha(plain_text[i])) {
      // If current key char is uppercase, keyReferenceValue would be 'A',
      // otherwise 'a' We use keyReferenceValue to convert key char to ascii
      //
      char keyReferenceValue = (isupper(key[char_index % key_len])) ? 'A' : 'a';

      // Similarly, we use referenceValue to convert plain text char to ascii,
      // it would
      //
      char referenceValue = (isupper(plain_text[i])) ? 'A' : 'a';
      shift_len = key[char_index % key_len] - keyReferenceValue;
      encrypted_stream << char(
          ((plain_text[i] - referenceValue + shift_len) % 26) + referenceValue);
      ++char_index;
    } else {
...
std::string decrypt_data(std::string encrypted_text, std::string key) {
  ...
  for (unsigned int i = 0; i < text_len; ++i) {
    if (isalpha(encrypted_text[i])) {
      ...
      shift_len = key[char_index % key_len] - keyReferenceValue;
      decrypted_stream << char(
          (((encrypted_text[i] - referenceValue) + (26 - shift_len)) % 26) +
          referenceValue);
      ++char_index;
    } else {
