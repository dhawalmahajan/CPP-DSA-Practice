...
void reorderList(Node* head) {
  ...
  if (head->next != NULL && head != NULL) {
    ...
    /* now prev is pointing the middle node of list and fast is pointing the
       last Node of the list, split the list into two parts and new head will
       point the 2 part */
    prev->next = NULL;
    new_head = slow;
    fast = NULL;
    // will slow is not equal to NULL means the new_head!=empty run the loop and
    // reverse the List
    while (slow) {
      t = slow->next;
      slow->next = fast;
