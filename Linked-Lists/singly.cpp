...
class SinglyLinkedList {
 ...
 public:
  ...
  void insertAtEnd(T data) {
    ...
    // if list is not empty, then put the address of the new node in the
    // tail->next and finally, make the next of newNode = NULL and tail will be
    // pointing the the newNode as now it is in the last position.
    tail->next = newNode;
    newNode->next = NULL;
    tail = newNode;
  }
  ...
  T front() {
    if (this->head == nullptr) throw std::runtime_error("Empty List.");
...
void menu() {
  ...
  while (true) {
    ...
    try {
      switch (ch) {
        case 1:
          insert_element(list, false);
          break;
        case 2:
          insert_element(list, true);
          break;
        case 3:
          list.printList();
          break;
        ...
        case 8:
          return;
        default:
          std::cout << "That's an invalid option. Please try again.\n";
      }
