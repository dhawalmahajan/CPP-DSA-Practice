...
// stock span values
void calculateSpan(const vector<int> price, const int priceSize,
  ...
  for (int priceIt = 1; priceIt < priceSize; priceIt++) {
    ...
    // top of stack
    stockSpan.push_back((spanStack.empty()) ? (priceIt + 1)
                                            : (priceIt - spanStack.top()));
...
void printArray(const vector<int>& arr, const int arrSize) {
  for (int arrIt = 0; arrIt < arrSize; arrIt++) cout << arr[arrIt] << " ";
}
