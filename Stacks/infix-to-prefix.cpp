/*
    Program to convert a infix expression to prefix expression a.k.a Polish
   Notation. Infix  expression : An expression in which the operator appears
   between the two operands. Prefix expression : An expression in which the
...
/*
   ...
   3. Reverse the infix expression i.e A+B*C will become C*B+A. Note while
   reversing each ‘(‘ will become ‘)’ and each ‘)’ becomes ‘(‘.
   4. Obtain the “nearly” postfix expression of the modified expression i.e
   CB*A+.
...
int main() {
  ...
  while (n--) {
    ...
    cin >> infix;
    cout << "Prefix Expression of " << infix << " is: " << infixToPrefix(infix)
         << endl;
  }
}
/*
