...
bool isBalanced(string s) {
  ...
  /*
      ...
      4. If stack is not empty as well st.top()+2 ==(int) data (we will assume
     that st.top '[' or '}' so by adding two in it should be ']' pr '}' ) or
     st.top+1 (we will assume that st.top be '(' so +1 would be ')')
      ...
      8. If the 7 step didn't run then return 0 means the expression is not
     balanced
      */
  for (i = 0; i < s.size(); i++) {
    data = s[i];
    if ((st.empty() != 1) &&
        ((int)(st.top() + 2) == (int)data || (int)(st.top() + 1) == (int)data))
      st.pop();
    else
      st.push(data);
  }
