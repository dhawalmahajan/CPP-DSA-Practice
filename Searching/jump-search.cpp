...
int jumpSearch(int arr[], int n, int key) {
  ...
  while (arr[end] <= key && end < n) {
    ...
    if (end > n - 1) end = n;  // if right exceeds then bound the range
  }
  for (int pos = start; pos < end; pos++) {  // perform linear search
    if (arr[pos] == key) return pos;         // the correct position of the key
  }
  return -1;  // if element is not found
}
// recursive jump search function to search data in an array
int recursive_jump_search(int *arr, int start, int end, int n, int to_search,
                          int jump) {
  // if end index of n is greater than n that means data not found so return -1
  if (end > n) return -1;
  // is the element to search is greater than end index and end index is less
  // than n than we need to jump
  else if (arr[end] < to_search) {
    start = end;
    end = end + jump;
  }
  // if else block run means data is between start and end index so then search
  // linearly
  else {
    // until start is less or equal to end run the loop
    while (start <= end) {
      // if data found then return the data index number
      if (arr[start] == to_search) return start;
...
int main() {
  ...
  if ((loct = recursive_jump_search(arr, 0, n - 1, n, key, (int)sqrt(n - 1))) !=
      -1)  // Calling of recursive jump Search function and getting a location
           // back
    cout << "Element found at location: " << loct << endl;
  else
    cout << "Element does not exist in the array." << endl;
