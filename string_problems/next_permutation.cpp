 ...
 * possible answers, find the lexicographically smallest one. Example: ab --> ba
 *  bb --> bb
 *  hefg --> hegf
 *  dhck --> dhkc
 *  dkhc --> hcdk
...
std::string next_permutation(std::string str) {
  int len = str.length();
  int i = len - 1;
  // We will iterate string from end, and once we have encountered a pair of
  // letters such that str[i] < str[i-1] , i-1 would become our pivot
...
int main() {
  ...
  std::cout << "Next permutation of " << str << " is " << str_next << std::endl;
  return 0;
}
