/*
 * Given a string pattern(P) and large Text string (T), Write a function search(
 * P , T) which provide all the occurrences of P in T. example : T =>
 * "AABAACAADAABAAABAA". P => "AABA" Output : 0, 9, 13 ( all indices of T where
 * pattern string P is starts to match.
 *
 * Approach:
 ...
 *         Step 2 : Then we will calculate hash of text portion from T[0-->M-1].
 * lets say  t(0) Step 3: if ( p == t(0) ) if they match, add it to list of
 * occurrences. Step 4: Go back to step 2, and calculate t(1) i.e hash of
 * T[1-->M] using t(0) in O(1).
 *
 *         The question remains, how do we calculate t(1) from t(0) in O(1), we
 * do it using Horner's rule H[m] = X[m]+ 10 (X[m-1] + 10(X[m-2]+……10(X[2] + 10
 * X[1]….))) —-> The 10 is the number of characters
 *
...
std::vector<int> search(const std::string pattern, const std::string text) {
  ...
  std::vector<int>
      indices;  // store all the indices of text where pattern matched.
...
void printIndices(std::vector<int> indices, const std::string pattern,
                  const std::string text) {
  if (indices.size() == 0) {
    std::cout << "\"" << pattern << "\" does not occur in \"" << text << "\""
              << std::endl;
  } else {
    std::cout << "\"" << pattern << "\" occurs in \"" << text
              << "\" at following position(s):";
    for (auto i : indices) {
      std::cout << i << " ";
    }
    std::cout << std::endl;
