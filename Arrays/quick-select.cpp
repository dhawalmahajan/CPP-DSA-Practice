// CPP program for implementation of "Quick-select" method based on Quick-sort
// or partition algorithm
// The Question here is to find the kth smallest element in an array.
// if array = {10 20 30 40 50 60} and k=2 then k'th smallest element will be :
// 20
...
int kthSmallest(int arr[], int left, int right, int k) {
  ...
  if (k > 0 && k <= right - left + 1) {
    ...
    // for left subarray
    if (index - left > k - 1) return kthSmallest(arr, left, index - 1, k);
