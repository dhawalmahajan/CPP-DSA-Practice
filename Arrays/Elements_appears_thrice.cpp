...
int main() {  // main function begins here
  ...
  for (int i = 0; i < size; i++) {
    ...
    cin >> arr[i];
    mp[arr[i]]++;  // this simply stores the frequency of each element in this
                   // array
  }

  map<int, int>::iterator
      it;  // this is an iterator which is used to iterate all over the map
  ...
  for (it = mp.begin(); it != mp.end(); it++) {
    if ((*it).second == 3)  // checks whether element frequency is 3 or not
    {
      ...
      cout << (*it).first
           << " ";  // print the element which satisfy the above condition
    }
  }
  if (count == 0) {
    cout
        << "There is no elements in this array whose frequency is 3.";  // special
                                                                        // print
                                                                        // if
                                                                        // there
                                                                        // is no
