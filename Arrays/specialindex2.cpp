...
special index-> if after deleting element from index i , sum of even index=sum
of odd index approach-> after deleting ,previous element at odd index will be
now at even index and vice versa s_odd= odd[0 to i]+ even[i+1 to len]
...
int main() {
  ...
  for (int i = 0; i < num; i++) {  // checking whether special index or not
    ...
    if (s_even ==
        s_odd) {  // checking whether sum of even index=sum of odd index
