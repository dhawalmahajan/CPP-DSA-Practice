...
// arr1[]={1,2,3,4,7}
// arr2[]={8,9,10}
// Time complexity: O(n1*m1) ,n1 is for linear traversal and m1 is for
// reordering. space complexity:O(1)
#include <bits/stdc++.h>
using namespace std;
...
void merge_array(int arr1[], int arr2[], int n1, int n2) {
  for (int i = 0; i < n1; i++) {
    // we will check the condition if the element of array1 is greater than
    // array2, if true ,simply swap them.
    if (arr1[i] > arr2[0]) {
      int temp = arr1[i]; /* logic for swapping the elements */
      arr1[i] = arr2[0];
      arr2[0] = temp;
      /*	swap(arr1[i],arr2[0]); this is stl function to swap the
       * element*/
      // after swapping the array element we need to sort the element of second
      // array because array is sorted.
      int key, j;
      for (int i = 1; i < n2; i++) {
        key = arr2[i];
        j = i - 1;
