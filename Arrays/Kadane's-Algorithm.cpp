...
int max_subarray_sum_method_1(int arr[], int n) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i; j < n; j++) {
      ...
      for (int k = i; k <= j; k++) {
        sum +=
            arr[k];  // taking sum of elements from ith position to jth position
      }
...
int max_subarray_sum_method_2(int arr[], int n) {
  ...
  for (int i = 0; i < n; i++) {
    sum = 0;
    for (int j = i; j < n; j++) {
      sum += arr[j];  // as we are just adding an element in our subarray so we
                      // can remove the k loop.
...
int main() {
  ...
  int ans1 = max_subarray_sum_method_1(
      arr, n);  // this is brute force approach time complexity=O(n^3) || space
                // complexity=O(1);
  int ans2 = max_subarray_sum_method_2(
      arr, n);  // this is optimized brute force approach time complexity=O(n^2)
                // || space complexity=O(1);
  int ans3 = max_subarray_sum_method_3(
      arr, n);  // this is further optimized approach called "Kadane's
