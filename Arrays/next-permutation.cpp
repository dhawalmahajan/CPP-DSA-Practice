...
vector<int> nextPermutation(int n, vector<int> nums) {
  ...
  // Base Case if vector have  size 2,simply reverse them suppose we have
  // nums=[1,2] then output will be [2,1].
  if (nums.size() == 2) {
    reverse(nums.begin(), nums.end());
  }
  // case where the size of vector is greater the 2 start traversing from the
  // right side
  //  While going from right to left, if we find an index whose value is less
  //  than index at right
  ...
  // Next thing that we can observe is the value at nums[change] is always
  // replaced by the value
  // which is greater than value at change index and occurring first while
  ...
  // After swapping we are just sort the values from change+1 index to n-1
  // index.
