...
int searchInsert(vector<int>& nums, int target) {
  ...
  // returning end+1 so that we can get the index where the target element can
  // be inserted
  return end + 1;
}
...
int main() {
  ...
  int target;
  cin >> target;
  cout << "The target element can be inserted at: "
...
Output: 1
(Since the target element is not present in the nums array, it will return the
position where the target element can be inserted) We can insert 2 at position 1
- [1,2,3,5,6]
