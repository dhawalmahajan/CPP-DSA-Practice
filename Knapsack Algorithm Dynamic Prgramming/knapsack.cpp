...
// Top-Down approch
int solveUsingMemoization(vector<int> &weight, vector<int> &value, int index,
  ...
  if (weight[index] <= capacity) {
    include =
        value[index] + solveUsingMemoization(weight, value, index - 1,
...
// Bottom-Up approach
int solveUsingTabulation(vector<int> &weight, vector<int> &value, int n,
                         int capacity) {
...
int main() {
  ...
  int n = weight.size();
  int maxWeight = 4;
  cout << "Maximum value in knapsack: " << knapsack(weight, value, n, maxWeight)
       << endl;
  return 0;
}