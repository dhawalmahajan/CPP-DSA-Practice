// Rod Cutting Problem
// Given a rod of length n and a list of rod prices of length i, where 1 <= i <=
// n, find the optimal way to cut the rod into smaller rods to maximize profit.

// Rod Cutting Optimal Approach
//     We will solve this problem in a bottom-up manner. (iteratively)
//     In the bottom-up approach, we solve smaller subproblems first, then move
//     on to larger subproblems. The following bottom-up approach computes
//     dp[i], which stores maximum profit achieved from the rod of length i from
//     1 to len. It uses the value of smaller values i already computed.
...
int RodCutting(vector<int> &prices, int len) {
  // (dp) stores the maximum revenue achieved from cutting a rod of length (from
  // 1 to len)
  ...
  for (int i = 1; i <= len; i++) {
    ...
    for (int j = 0; j < i; j++) {
      // (dp[i]) stores the maximum revenue achieved from cutting a rod of
      // length (i)
...
//        rod length = 8
//        Best: Cut the rod into two pieces of length 2 and 6 to gain revenue of
//        5 + 17 = 22

//     3. prices[] = [3, 5, 8, 9, 10, 17, 17, 20]
//        rod length = 8
//        Best: Cut the rod into eight pieces of length 1 to gain revenue of 8 *
//        3 = 24
