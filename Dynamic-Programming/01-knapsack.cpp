...
int knapsack(int wait[], int price[], int n, int capacity,
  ...
  // base condition - when the bag capacity is 0 and wait[] and price[] size is
  // 0.
  if (n == 0 || capacity == 0) return 0;

  // for each call first check the table if value are present(not -1) then
  // directly return
  ...
  if (wait[n - 1] <= capacity) {
    dp[n][capacity] = max(
        price[n - 1] + knapsack(wait, price, n - 1, capacity - wait[n - 1], dp),
