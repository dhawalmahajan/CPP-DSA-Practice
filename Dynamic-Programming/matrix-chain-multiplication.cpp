...
int matrixChainMulti(vector<int> &arr, const int i, const int j,
  ...
  for (int k = i; k < j; k++) {
    int temp_ans = matrixChainMulti(arr, i, k, dp) +
                   matrixChainMulti(arr, k + 1, j, dp) +
                   (arr[i - 1] * arr[k] * arr[j]);
    if (temp_ans < countMin) countMin = temp_ans;
  }
