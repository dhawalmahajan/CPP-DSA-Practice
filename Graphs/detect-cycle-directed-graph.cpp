...
bool detectCycle(int node, vector<vector<int>> &adj, vector<bool> &path,
                 vector<bool> &visited) {
  visited[node] = true;
  path[node] = true;
  ...
  for (int j = 0; j < adj[node].size(); j++) {
    ...
    if (path[neighbor] == true) return true;
...
bool isCyclic(int V, vector<vector<int>> &edges) {
  ...
  for (int i = 0; i < V; i++) {
    if (!visited[i] && detectCycle(i, adj, path, visited)) return true;
  }
  return false;
}
