...
Criteria:
1: If a string contain 3 more than three constant or more than 5 vowels
continuously than it is a bad string else good 2: '?' can  be replaced by any
alphabets means it can be both constant and vowel
*/
#include <iostream>
#include <string>
using namespace std;
...
int Bad_or_Good(string s1) {
  ...
  for (int i = 0; i < s1.length(); i++) {
    ...
    // if the s[i]th character is not a vowel nor '?' that means it is a
    // constant
    else {
