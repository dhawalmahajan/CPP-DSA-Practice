...
void PrimeSieve(int n) {
  ...
  fill(sieve.begin(), sieve.end(),
       1);  // Fill the array with 1s, which means all numbers are initially
            // assumed to be prime.
  sieve[0] = 0;  // Exception:Zero is not a Prime
  sieve[1] = 0;  // Exception:One  is not a Prime
  for (int i = 2; i * i <= n; i++) {
    if (sieve[i] ==
        1) {  // This condition will only be executed if the sieve[i] is prime,
              // mark all the multiples of Prime Numbers as non-prime.
