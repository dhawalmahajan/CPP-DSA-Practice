...
int main()  // Main function
{
  ...
  for (int x = (num_str.size() - 1); x >= 0; x--) {  // for loop started
    new_str += num_str[x];  // assigning the value input by user to new variable
                            // in reverse order
  }  // for loop end

  cout << (num_str == new_str
               ? "palindrome"
               : "Non-palindrome");  // checking whether the value assigned to
                                     // both variables is equal or not using
                                     // ternary operator and printing whether
                                     // it's palindrome or non-palindrome
