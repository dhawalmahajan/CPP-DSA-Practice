...
positive integers less than n that are coprime to n. That is, totient(n) is the
number of m∈N such that 1≤m<n and gcd(m,n)=1. You are given an integer N and are
supposed to find the value of Euler toient function for N.
...
2)Calculate the numerator and denominator i.e N*(p1-1)(p2-1)...(pn-1) and
(p1*p2*p3...*pn) 3)Divide numerator by denominator.
