...
// We use a single Odd function to check for Even-Odd, If it return true then it
// is Odd else it is Even.
bool isOddBin(int num) {
  // We compare the last bit of the number to decide for Even or Odd
  // If the last bit of the num is set (i.e. 1) then number is odd and its
  // "Binary And (&)" with the 1 will return true(1). Similarly in case of even
  // num last bit is unset/off (i.e. 0) and its "Binary And (&)" with the 1 will
  // return false(0).
  return (num & 1);
}
...
int main() {
  ...
  result =
      isOddBin(num)
          ? "Odd"
          : "Even";  // Evaluating string based on result from isOdd function
