...
TreeNode* find(TreeNode* root, const int& data) {
  ...
  if (root == nullptr) {
    throw std::runtime_error(
        "Error: find() cannot find the data. The data doesn't exist.");
...
void Insert(TreeNode*& root, const int& data) {
  ...
  if (root == nullptr) {
    ...
    throw std::runtime_error("The node already exist. Duplicates not allowed");
  } else if (root->data < data) {
...
bool perfect_recursive(TreeNode* cur, int depth, int level = 0) {
  ...
  if (cur->left != nullptr && cur->right != nullptr) {
    return perfect_recursive(cur->left, depth, level + 1) &&
           perfect_recursive(cur->right, depth, level + 1);
  }
  return false;
}
...
void leaf_nodes(TreeNode* root)  // Print all leafnode in BST
{
  ...
  if (root->left) leaf_nodes(root->left);

  if (root->right) leaf_nodes(root->right);
}
...
int main() {
  ...
  if (isfull(root)) {
    std::cout << "The binary tree is FULL" << std::endl;
  } else {
    std::cout << "The binary tree is not FULL" << std::endl;
  }
  ...
  n = f_max(root);
  if (n) {
    std::cout << "Maximum Value Present in the tree is : " << n->data
              << std::endl;
  }

  n = f_min(root);
  if (n) {
    std::cout << "Minimum Value Present in the tree is : " << n->data
              << std::endl;
  }
