// The sum of right leaves algorithm uses a queue to navigate through a binary
// tree Worst Case Time Complexity: O(n) Average Time Complexity: O(n)
...
int sumRightLeaves(Node* root) {
  ...
  while (!q.empty()) {
    ...
    if (q.front()->right) {
      ...
      if (q.front()->right->right == nullptr &&
          q.front()->right->left == nullptr) {
