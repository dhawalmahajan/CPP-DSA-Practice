...
void merging(int arr[], int l, int h, int m) {
  ...
  int k = l;
  while (i < N1 &&
         j < N2) {  // Inserting back the elements from left and right array
    if (left[i] <= right[j]) {  // If left element is smaller than the right one
                                // then we push
  ...
  for (; i < N1;
       i++) {  // These loop executes only when there is difference in size of
...
void merge_sort(int arr[], int l, int h) {
  if (l < h) {
    ...
    merge_sort(
        arr, l,
        mid);  // Recursively calling for elements before mid(including mid)
