...
class QueueLL {
 ...
 public:
  QueueLL()  // Empty constructor to initialize front and rear to NULL because
             // in the start the queue will be empty
  {
    front = rear = NULL;
    _length = 0;
  }
  ...
  // Function `Dequeue` to remove an item from the queue from the front side
  // (Start)
  void dequeue() {
    Node *temp = front;
    front = front->next;
