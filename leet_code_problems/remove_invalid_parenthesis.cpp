 ...
 * 2. They should be in right order, for each open there has to be a close, so
 ...
 * When we reach end of string, we check if it is valid by left, right and open
 * count and push the string to result set.
 * 4. If a character is not a parenthesis operator, we simply add it to the
 * result string.
 * 5. At the end of backtracking, we simply replace the current result string
 * with the one we started this iteration with.
 *
...
void dfs(const std::string& s, std::string temp, int index, int left, int right,
         int open, std::unordered_set<std::string>& result) {
  if (index == s.length()) {
