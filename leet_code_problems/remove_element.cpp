 ...
 * Approach:
 *
 * The idea is to use two pointers, one iterates the entire nums(i), and the
 * other one maintains the length of set of numbers which we want in our
 * output.(j). The idea is everytime we find a wanted element we move the j.
 * This way j maintains all the wanted element at top of the nums.
 */
